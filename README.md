...
CodeDoc 把软件著作权源程序材料的整理过程收进一套本地向导：选择代码文件，设置清洗与排版规则，逐页检查生成结果，最后完成风险校验并导出 PDF、docx 和 txt 准备稿。

> **使用流程：** 导入本地项目 → 筛选并排序代码文件 → 清洗与规范化排版 → 分页预览 → 校验并导出

### 1. 选择需要整理的代码

按真实目录结构勾选需要纳入材料的文件，调整文件顺序，并通过代码行数、预计页数和文件类型统计确认材料范围。

![文件筛选、排序与代码量统计界面](design/snapshot/step-2.jpeg)

### 2. 设置清洗与排版规则

填写软件全称、版本号和著作权人名称，按需删除注释与空行、脱敏敏感信息、折断超长行，并实时对比清洗前后的代码。

![代码清洗、脱敏与排版设置界面](design/snapshot/step-3.jpeg)

### 3. 检查最终分页效果

在应用内逐页查看生成效果，核对页眉、页码、代码行数以及前后段边界，确认材料内容和版式符合预期。

![源程序材料分页预览界面](design/snapshot/step-4.jpeg)

### 4. 管理全局扫描设置

统一维护所有项目共用的扫描排除规则，避开依赖、构建产物和缓存目录；同时可查看隐私说明、应用版本与更新状态。
