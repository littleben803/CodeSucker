...
- README 和设计文档直接使用同一品牌母图；不得在 `docs/brand/` 或其他目录维护内容相同的 Logo 副本。

正式发布截图待应用界面完成验收后重新制作。仓库目录、远程仓库和发布产物继续使用当前技术标识，任何改名都必须作为独立兼容性任务处理。
...
- CodeDoc 正式发布截图。
- Windows 正式代码签名方案。
- 如果未来变更仓库、App ID、技术产品名或发布来源，需要单独制定兼容与迁移方案。
